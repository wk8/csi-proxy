...
// runGenerator runs csi-api-gen with the given CLI args, and returns stdout and stderr.
...
func runGenerator(t *testing.T, testName string, cliArgs ...string) (string, string) {
	stdoutFile, err := ioutil.TempFile("", "test-csi-api-gen-stdout-"+testName)
	require.Nil(t, err)
	stderrFile, err := ioutil.TempFile("", "test-csi-api-gen-stderr-"+testName)
	require.Nil(t, err)
